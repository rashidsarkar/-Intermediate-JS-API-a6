...
const dataHandle = async () => {
  ...
  // Call the tabHandler function with the fetched data
  tabHandler(data);
};

// Function to handle tab-related elements
const tabHandler = (datas) => {
  // Call the videoHandler function with a default video ID
  videoHandler("1000");
  console.log(datas);

  // Loop through the data and create tab elements dynamically
  datas.forEach((data) => {
  ...
  // Select and highlight the first tab
...
const handleTabClick = (tab) => {
  ...
  // Add the 'tab-active' class to the clicked tab and change its background color
  tab.classList.add("tab-active");
  tab.style.backgroundColor = "#FF1F3D";
};

// Event listener for tab elements
const tabEls = document.querySelectorAll(".tab");
tabEls.forEach((mytab) => {
  mytab.addEventListener("click", () => {
    const activeTab = document.querySelector(".tab-active");

    // Remove 'tab-active' class from the previously active tab
    if (activeTab) {
...
const videoHandler = (videoID) => {
  const videoHandler = async () => {
    ...
    if (data.length === 0) {
      ...
      // Loop through the video data and create video elements dynamically
      data.forEach((item) => {
        const div = document.createElement("div");

        // Check if the author is verified and set an image accordingly
...
sortVewelement.addEventListener("click", function () {
  // Sort the showData array based on views
  showData.sort((a, b) => {
...
// Call the dataHandle function to start fetching and displaying data
dataHandle();
